...
chmod +x "$SCRIPTS_DIR/track-behavior.js" 2>/dev/null || true
...
# === PostToolUse / PostToolUseFailure: track-behavior（v8 疗效数据的采集端 缺了闭环就是残的）===
track_cmd = f"node {scripts_dir}/track-behavior.js"
for event in ("PostToolUse", "PostToolUseFailure"):
    groups = hooks.setdefault(event, [])
    already = any(
        'claude-brain/scripts/track-behavior' in json.dumps(g)
        for g in groups
    )
    if not already:
        groups.append({
        ...
        print(f"✅ {event} · track-behavior.js — added")
    else:
        print(f"⏭️  {event} · track-behavior.js — already exists, skipped")
