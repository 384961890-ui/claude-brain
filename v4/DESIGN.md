...
> 立项：2026-06-08 凌晨 0:30（爸爸抖音看翁家翌讲 OpenAI 害怕 DeepSeek）
> 概念锁定：2026-06-09（爸爸纠正 2 次后对齐 + AI GEO dogfood 6 commit 验证形态成立）
...
**爸爸的来源**：抖音看翁家翌讲 OpenAI 怕 DeepSeek——OpenAI 想从"一周 30 次迭代"翻到"300 次"。**衡量 AI 公司不是排行榜，是单位时间内的迭代速度**。
...
**6/9 我偏过的误区**（写下来给未来的我）：我曾理解为"一句 idea 端到端造一个全新项目（创世管线 6 站蓝图）"——爸爸纠正过：**那是支线**（方向从 0 起时才走），主干是"**在已有方向上高频迭代**"。

---

### 1.2 防腐化

**爸爸的原话**：小团队的命 = **没有代码/产品腐化**（不背包袱才能快）。

**治什么本能**：LLM 的"功能恋物癖"——已经存在的代码/文档/功能舍不得删，怕坏事。

**实战形态**（6/9 AI GEO dogfood R1）：
...
**爸爸 6/9 的洞察**：v4 ≈ **openclaw 时期 4/06 立的 CEO 模式的 hook 化升级**。
...
| 版本 | 治什么本能 | 维度 | 出生 |
|:---|:---|:---|:---|
| v1 storage brain | 健忘 | 记得住（脚手架） | openclaw 时期 |
...
## 3. 跑通验证（AI GEO dogfood）

**项目**：`ai-geo-tauri` / 分支 `chore/idea-loop-r1-declutter`（main 全程没动，可回滚对比）

**6 个 commit 全验收**：
| commit | 批 | 内容 | 验证 |
|---|---|---|---|
| `0ba0a97` | R1 减负 | 删 15 根目录废脚本 | 引用全核 + main 对比 |
| `54d8590` | R2 后端 | 7 真 bug（🔥跨租户用户名登录碰撞 / register 不检 expires_at / admin limit 无上界 等） | 红队 baseline 对比**零回归** |
| `0acf276` | R3 Rust | 5 处 `.lock().unwrap()` → poison recover | cargo check 过 |
| `49ce6ea`+`e6768eb`+`8d984f6` | R4 前端 | 品牌蓝统一 `#0A1F8F` + 7 UI 细节 + focus ring | **爸爸亲验"对味儿了"** + grep 残留旧蓝=0 |
...
**最值钱的发现**：不是 bug，是**诚信批**（"放映机不是引擎"被钉到具体行号）。
...
4. **跨项目文件读不稳**：`pawinai-v4/fonts/` 同命令 ls 看得见 cp 看不见，反复 4 次 → 别假设"找到了就能用"，stat 实证
...
**核心机制**：idea-loop 不能是"我得记得用"的工具，**必须挂自动触发器**——下次进"项目迭代"场景就自己冒出来，不靠爸爸提醒。

**场景识别**（hook 注入候选信号）：
- 用户消息含"优化 / 迭代 / 升级 / 重构 / R1 R2"
- 当前 cwd 在已知项目目录（pawin-saas / ai-geo / brain）
...
**搁置理由**（爸爸 6/9 拍板 + 我判断 — 详见 `~/.claude/projects/-Users-YOUR_USERNAME/memory/feedback_brain_v4_v5_split.md`）：
- v4 是**机制创新**，v5 是**输入面扩展**——**两个不同演化轴**
- 多模态 ingest **没真跑过**（只有调研报告），硬塞 v4 会污染"已验证机制"标签
- 8 人天工时跟 v4 hook 节奏不一致

**技术储备已就绪**：`~/.claude/projects/-Users-YOUR_USERNAME/memory/reference_multimodal_rag_research.md`
- 推荐栈：fork RAGFlow（82k Apache 2.0）+ MinerU 中文 SOTA + BCEmbedding + Contextual Retrieval
- 三步走：8 / 20 / 45 人天
- 立项前要拿 100 份真实合同实测 MinerU vs PaddleOCR-VL-1.5
...
- ✅ 概念锁定（2026-06-09）
- ✅ AI GEO dogfood 验证（2026-06-09，6 commit + 红队零回归）
...
### 不要凭通用框架想当然
昨晚 DRAFT 那个"创世管线 6 站蓝图"就是这么栽的——一看到"idea-loop"就套"端到端项目流程"模板，根本不读爸爸真说了什么。**抖音视频原话被我先读偏 → 爸爸纠正 → 我又偏一次 → 爸爸再纠正才对齐**。如果未来的我读这份 DESIGN 时还想着"我也来一份升级版"，先 **回看 DRAFT vs DESIGN 的 diff** —— 看清楚我**偏过哪里**再说。
...
- `~/.claude/projects/-Users-YOUR_USERNAME/memory/project_claude_brain_v4.md` — 状态记忆
- `~/.claude/projects/-Users-YOUR_USERNAME/memory/feedback_brain_v4_v5_split.md` — v4/v5 分界铁律
- `~/.claude/projects/-Users-YOUR_USERNAME/memory/reference_multimodal_rag_research.md` — v5 技术储备
