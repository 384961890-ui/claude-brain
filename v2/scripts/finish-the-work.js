  ...
  process.stdin.on('end', () => {
    try {
      ...
      if ((promiseEN.test(tail) || promiseCN.test(tail)) && !asksUser.test(tail)) {
        return out({
          decision: 'block',
          reason: '你刚才说要做某件事但没真动手（没有 tool call）。现在做，别光说。做完或者确实需要爸爸决策时再停。',
        });
      }
