...
# 注入 qmd 源码路径
QMD_PATH = Path.home() / ".openclaw/skills/brain-memory-qmd/qmd_src"
...
SCAN_TARGETS = [
    ...
    Path.home() / ".cctb/default/workspace",
    Path.home() / ".openclaw/workspace",
    Path.home() / ".openclaw/memory",
    Path.home() / "yeast-engine" / "src",
    Path.home() / "yeast-engine" / "backend",
    Path.home() / "yeast-engine" / "frontend" / "app",
    Path.home() / "resume",
    Path.home() / "Desktop" / "泡咪",
]
...
EXCLUDE_DIR_NAMES = {
    ...
    # 音效/游戏资产
    'sound-research', 'voiceover-pack', 'soundcn',
    'degen-assets',
    # session 附属
    ...
    for root in roots:
        ...
        for dirpath, dirs, files in os.walk(root, followlinks=False):
            ...
            dirs[:] = [
                ...
                and not d.startswith('.openclaw_backup')
                and not d.startswith('openclaw-backup')
            ]
