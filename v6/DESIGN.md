...
> 立项：2026-06-16 白天深聊（爸爸问 opus4.8 三毛病 + 屎山"能跑就行"怎么从根治）
...
> 普通 harness 是爸爸在外面写死的规则（外挂的护甲）；v6 靠 lessons 能从我的伤疤里自己长出来——**从"外挂"变成"免疫"**。
...
爸爸定调"软提醒为主 + 极少铁律硬拦"。但六维审查 + 我翻车证实：**PostToolUse 里绝不能用 `decision:block` 硬拦**——
...
1. **挂 hook 太早**：未验证完就挂 settings，还在同 session 继续动文件 = 给自己埋雷，被爸爸喝止。
...
- ⏸️ 免疫闭环：屎山被爸爸抓到 → capture-lesson 自动写 lesson → inject-context 下轮提前提醒（外挂变免疫的完整体）
...
**6/17 凌晨 v8 误命名事件**：我把"第二支 loop"误命名为 v8（爸爸纠正）。根因——训练数据里"软件版本号递增"的偏见让我看到新功能就想新版本号。**v6 是一个轴不是一个点**——同一个 v6 命题（给延迟的好装即时红灯）下可以长多支，不开新版本号。
...
├── LOOP-CHECKLIST.md               爸爸快速参考（应急步骤）· 越权事故的产物
...
- `~/.claude/projects/-Users-YOUR_USERNAME/memory/feedback/feedback_subagent_writes_unauthorized_files.md`（v8 越权事件根因）
- `~/.claude/projects/-Users-YOUR_USERNAME/memory/feedback/feedback_claude_code_5h_limit_fixed_cycle.md`（5h 周期规律 · 第二支调度前提）
